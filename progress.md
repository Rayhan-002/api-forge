...
### Revision — real press-and-hold drag (no dedicated handle) + tooltips everywhere

User feedback after trying it: (1) the dedicated grip-handle icon for dragging requests wasn't wanted — pressing and holding anywhere on the request or folder row itself, moving it, and releasing over the target should just work, Postman-style, for **both** requests and folders (folder-to-folder dragging hadn't been built at all — only the Move-folder dialog had); (2) icon-only buttons need hover tooltips everywhere in the app, not just the one this was first raised about, since a new user has no way to know what an unlabeled icon button does.

- [x] Removed the `GripVertical` handle button from request rows. `useDraggable`'s listeners now attach directly to the row's root element (for both request rows and folder header rows), relying on the existing pointer-sensor activation distance (4px) to tell a plain click from a drag — a tap opens the request/toggles the folder as before, a press-and-move drags it, matching how dnd-kit's own sortable-list examples handle a row that's both clickable and draggable.
- [x] Folder rows are now `useDraggable` *and* `useDroppable` on the same element (a standard dnd-kit pattern for tree UIs), so a folder can be dragged directly onto another folder or collection to reparent it — the drag path reuses the exact same `moveCollection` mutation the "Move folder" dialog already used, plus a client-side self/descendant guard (`getDescendantIds`) that skips a doomed API round-trip for the most common accidental drop, falling back to the backend's own cycle-check error message for anything it doesn't catch.
- [x] Every icon-only button nested inside a now-draggable row (expand/collapse, new subfolder, move, rename, delete) gets `onPointerDown={stopPropagation}` so dnd-kit's sensor never starts tracking a drag from a click that began on one of them — verified live that these still work correctly rather than assumed.
- [x] Added `title` tooltips to every icon-only (or otherwise non-obvious) control across the app: the dialog close button, assertion edit/delete, history restore/delete, the extraction-rule remove chip, environment row expand/rename/delete and variable edit/delete, the key-value row "enabled" checkbox and remove button, the topbar's sign-out button (which had **no** accessible label at all before this — `aria-label` was added alongside the `title`), and the environment switcher. The "Move folder" button the user said to keep now explains itself on hover too.
